...
- `halfadder.png` — half adder: G0 XOR -> S, G1 AND -> C
- `mux2to1.png` — gate-level 2-to-1 MUX: G1/G2 AND, G3 NOT, G4 OR, wire W1
- `decoder.png` — gate-level 2-to-4 decoder: 2 inverters, 4 enable-gated AND minterms,
  active-low outputs Y0..Y3
- `fourbitadder.png` — 4-bit ripple-carry adder block diagram: M0..M3, carry chain C0..C4
- `decoder_synth.png` — 2-to-4 decoder synthesized to gates (VeriSim Gates view); author-generated
- `fsm_baba_states.png` — BABA detector state diagram (IDLE->B_->BA_->BAB_, A/Y=1 feedback)
- `synthesis_flow.png` — Verilog RTL -> Yosys -> gate netlist -> FPGA bitstream (lecture stops at schematic)
- `tangnano_pinout.png` — Tang Nano 9K pinout drawn from scratch (clk 52; S1=4, S2=3; LEDs 10/11/13/14/15/16, active-low)
...
- `wave_halfadder.png`   — A,B sweep; S=A^B, C=A&B
- `wave_fulladder.png`   — all 8 input cases; Co,S
- `wave_mux4to1.png`     — y follows the selected input as sel steps 00->11
- `wave_glitch.png`      — static-1 hazard: Y dips while it should hold 1
- `wave_dff.png`         — Q captures D on rising edges only
- `wave_nonblocking.png` — pipeline: b -> a (1 clk) -> c (1 clk); c lags a
- `wave_baba.png`        — BABA Mealy detector: Y=1 on the fifth input
- `wave_mcu.png`         — fetch/decode/execute; acc 3->5->7->5; out=5, then halted
